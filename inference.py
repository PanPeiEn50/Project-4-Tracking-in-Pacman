...
def constructBayesNet(gameState: hunters.GameState):
    ...
    agent_positions = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    observation_range = list(range(X_RANGE + Y_RANGE - 2 + MAX_NOISE + 1))

    variableDomainsDict[PAC] = agent_positions
    variableDomainsDict[GHOST0] = agent_positions
    variableDomainsDict[GHOST1] = agent_positions
    variableDomainsDict[OBS0] = observation_range
    variableDomainsDict[OBS1] = observation_range
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        total_sum = self.total()
        if total_sum == 0:
            raise ValueError("Cannot sample from an empty or zero-sum distribution.")

        normalized_values = {key: value / total_sum for key, value in self.items()}
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Special case: if the ghost is in jail
        if ghostPosition == jailPosition:
            return 1 if noisyDistance is None else 0
        
        # Special case: if the observation is None but the ghost is not in jail
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(numParticles):
            self.particles.append(legalPositions[positionIndex])
            positionIndex = (positionIndex + 1) % len(legalPositions)
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
